...
public class GameService : IGameService
{
        ...
        Guid roundId, Guid playerId, Guid cardId)
    {
        ...
        if (round.CurrentTurnIndex < 0 ||
            round.CurrentTurnIndex >= turnOrder.Count ||
            turnOrder[round.CurrentTurnIndex] != playerId)
        ...
        // Generate signed URL for the revealed card
        var signedUrl = await _storageService.GetSignedUrlAsync(card.StoragePath);
    ...
    public async Task<List<MemeCardDto>> GetPlayerHandAsync(Guid playerId)
    {
        ...
        var result = new List<MemeCardDto>();

        foreach (var pc in cards)
        {
            var signedUrl = await _storageService.GetSignedUrlAsync(pc.MemeCard.StoragePath);
            result.Add(new MemeCardDto
            {
                Id = pc.MemeCard.Id,
    ...
    private async Task<List<MemeCard>> GetShuffledDeckAsync(Guid roomId, int count)
    {
